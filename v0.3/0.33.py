...
for cliente_afetado in clientes_afetados:
    try:
        cliente_afetado_cip = cliente_afetado.upper() # transforma em maiúsculas para comparação
        grupo = grupos_correspondentes.get_group(cliente_afetado_cip)
        email_grupo = grupo['INTERNET_E_MAIL'].values
        email = email_grupo[0] if len(email_grupo) > 0 else ''
    except KeyError:
        email = 'Sem contato na base de dados do CIP.'
    emails.append(email)
