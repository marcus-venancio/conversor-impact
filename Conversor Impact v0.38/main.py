...
df = pd.read_excel('IMPACTANALYSISS.xlsx', dtype=str)
clientes_afetados = pd.Series(df['CUSTOMER_ID'].astype(str).values)
circuito = df['NCO'].values
morada_cliente = df['MORADA_A'].values
...
contatos = pd.read_excel(r'\\Intranet.onitelecom.pt\customerservice\CGO\CIP - Intervenções Programadas\Contactos CIP Remedy.xlsx')
emails = []
